...
<script context="module">
  ...
  let pagePaths = Object.keys(pages).map((path) =>
...
<script>
  ...
  onMount(async () => {
    document.getElementsByClassName("project-title")[0].innerText = "bread 🍞";
  });
</script>
...
<div data-sveltekit-preload-data={prefetchStrategy} class="grid">
  ...
  {#if !$navigating}
    <main in:blur|local id="evidence-content">
      <div
        class="content"
        class:settings-content={$page.url.pathname.startsWith("/settings")}
      >
        <article
          class:settings-article={$page.url.pathname.startsWith("/settings")}
        >
          <slot />
          <p>&nbsp;</p>
        </article>
