...
t_attr as (
	select txhash, timestamp, type, convert_from(decode(attrs->>'key', 'base64'), 'UTF8') as key, convert_from(decode(attrs->>'value', 'base64'), 'UTF8') as value, attrs->>'index' = 'true' as index
	from type_attr, jsonb_array_elements(attributes) as attrs
),
dup_events as (
select txhash, type, key, array_agg(value) as values
from t_attr
